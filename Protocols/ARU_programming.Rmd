...
1. **Install flash cards**
    a. Insert 1 - 4 SDHC or SDXC cards (16, 32, 64, or 128 GB; Class 10), filling from Slot A - D as needed. If using an SDXC card, it will likely need to be reformatted from the default `exFAT` file system to the `FAT32` file system. Seek online assistance relevant to your operating system.
...
1. **Set location prefix and coordinates of ARU**
    ...
    c. Set the location prefix using the following convention: \<ARU ID\>-\<established point ID\>. 
        - For example, the prefix for audio collected on Cedar Island NWR at point `CDR4H` with SongMeter `IM06` would be `IM06-CDR4H`. Only 12 alphanumeric spaces are available, so abbreviating long, historical location names may be necessary.
