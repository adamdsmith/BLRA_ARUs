...
1. **Mount and secure the remote broadcast unit case to the T-post with wing nuts**
    ...
    c. Put the timer into automatic mode. A \manual{} icon should be indicated along the bottom of the display. If so, press the `MANUAL` button once to put the timer in automatic mode and display `AUTO` along the bottom of the display. If the unit is already in `AUTO` mode, press the `MANUAL` button twice to pass through `MANUAL` mode and back into `AUTO` mode. This step prevents a glitch in some timer units whereby the broadcaster turns on when battery pack power is first supplied.
