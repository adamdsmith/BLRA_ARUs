...
1. **Set the day of the week and time of the timer.**
    ...
    c. While holding the \myclock{} button, use the `D+` button to set the current day of the week, the `H+` button to set the current hour, and the `M+` button to set the current minute. If no button is pressed for 2 seconds while holding the \myclock{} button, you will need to release the \myclock{} button and push/hold it again to further adjust the day of week or time.
    
1. **Put the timer into manual mode.**
    a. If a \manual{} icon (some older models), or `ON` is indicated along the bottom of the display, proceed to the next step.
    b. If no \manual{} icon, or `AUTO` or `OFF` is indicated along the bottom of the display, press the `MANUAL` button until either the \manual{} icon (some older units) or `ON` appears along the bottom of the display (newer units).
