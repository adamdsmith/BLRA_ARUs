...
1. **Confirm or update the day of the week and time of the timer.**
    a. Visit [time.gov](https://time.gov/) for current official time.
    b. If the timer is locked (i.e., there is a \locked{} icon in the lower left corner of the display), press the `C/R` button 4X.
    c. While holding the \myclock{} button, use the `D+` button to set the current day of the week day, the `H+` button to set the current hour, and the `M+` button to set the current minute, as necessary. If no button is pressed for 2 seconds while holding the \myclock{} button, you will need to release the \myclock{} button and push/hold it again to further adjust the day of week or time.
    
1. **Put the timer into manual mode.**
    a. If a \manual{} icon is indicated along the bottom of the display, proceed to the next step.
    b. If `AUTO` is indicated along the bottom of the display, press the `MANUAL` button once to put the timer into manual mode and display the \manual{} icon along the bottom of the display.
...
1. **Set the remote broadcast unit program.**
    ...
    b. Every sequence available in step 3a requires scheduling the `ON/OFF` times for \underline{three} programs. They \underline{do not} necessarily occur on consecutive days. When programming a broadcast sequence, pay careful attention to the days of the week and the `ON/OFF` timing of the three programs.
    ...
    g. Press the \circled{P} button to advance and set the `OFF` time for `Program 1`, using an approach similar to that described in steps 3e - 3f. Set the `OFF` time for \underline{13 minutes after} the `ON` time.
    ...
    i. Repeat steps 3e - 3g to set the `ON` and `OFF` day of the week and times for `Program 2`.
    j. Repeat steps 3h - 3i to set the `ON` and `OFF` day of the week and times for `Program 3`.
